  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredHeightData = heightResponse.data.filter(item => item.data.startsWith('P8|Height:'));
        const filteredAreaData = areaResponse.data.filter(item => item.data.startsWith('P8|Area:'));
        const filteredStatusData = statusResponse.data.filter(item => item.data.startsWith('P8|Status:'));
        const filteredSoilData = soilResponse.data.filter(item => item.data.startsWith('P8|Soil:'));
        const filteredImageData = imagesResponse.data.filter(item => item.filename.startsWith('image_8.jpg'));
