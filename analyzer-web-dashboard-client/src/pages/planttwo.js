  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredHeightData = heightResponse.data.filter(item => item.data.startsWith('P2|Height:'));
        const filteredAreaData = areaResponse.data.filter(item => item.data.startsWith('P2|Area:'));
        const filteredStatusData = statusResponse.data.filter(item => item.data.startsWith('P2|Status:'));
        const filteredSoilData = soilResponse.data.filter(item => item.data.startsWith('P2|Soil:'));
        const filteredImageData = imagesResponse.data.filter(item => item.filename.startsWith('image_2.jpg'));
