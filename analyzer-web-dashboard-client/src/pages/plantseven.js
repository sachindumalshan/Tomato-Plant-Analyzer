  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredHeightData = heightResponse.data.filter(item => item.data.startsWith('P7|Height:'));
        const filteredAreaData = areaResponse.data.filter(item => item.data.startsWith('P7|Area:'));
        const filteredStatusData = statusResponse.data.filter(item => item.data.startsWith('P7|Status:'));
        const filteredSoilData = soilResponse.data.filter(item => item.data.startsWith('P7|Soil:'));
        const filteredImageData = imagesResponse.data.filter(item => item.filename.startsWith('image_7.jpg'));
