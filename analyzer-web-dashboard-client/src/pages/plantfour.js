  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredHeightData = heightResponse.data.filter(item => item.data.startsWith('P4|Height:'));
        const filteredAreaData = areaResponse.data.filter(item => item.data.startsWith('P4|Area:'));
        const filteredStatusData = statusResponse.data.filter(item => item.data.startsWith('P4|Status:'));
        const filteredSoilData = soilResponse.data.filter(item => item.data.startsWith('P4|Soil:'));
        const filteredImageData = imagesResponse.data.filter(item => item.filename.startsWith('image_4.jpg'));
